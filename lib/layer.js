...
Layer.prototype.update = function (ctx, dt) {
    // notify to update
    this.rootNode.tick(dt);
}

Layer.prototype.render = function (ctx, dt) {
    if (!this.visible) {
        return;
    }
    ...
    // render
    this.renderNode(this.rootNode, ctx, dt);
...
Layer.prototype.renderNode = function(node, ctx, delta) {
    var i;
