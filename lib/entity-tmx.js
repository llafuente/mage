...
EntityTMX.prototype.loadTilesets = function loadTilesets() {
    ...
    function load(err, image, src) {
        for (i = 0; i < max; ++i) {
            if (self.tilesets[i].image == src) {
                self.tilesets[i].image = image;
            }
        }
    ...
    for (i = 0; i < max; ++i) {
        ...
        Assets.loadImage(tileset.image, load);

        this.tilesets.push(tileset);
    }

    // clean-up: dont needed there
